...
# เริ่มด้วยการดึง NPC 1 ตัวเข้าโซนผู้เล่น
print("gather:", refl("ForceGather"))
for attempt in range(50):
    dump = refl("Dump")
    line = next((l for l in dump.splitlines() if "chibi@" in l), None)
    if attempt % 5 == 0: print("t",attempt,"|",line or "(no chibi)")
        ...
        if not shot.get("isError"):
            for item in shot.get("content", []):
                if item.get("type")=="image":
                    ...
                    print("SAVED walking shot:", len(png), "bytes |", line)
                    sys.exit(0)
    time.sleep(0.4)
