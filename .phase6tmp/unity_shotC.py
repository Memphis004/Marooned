...
def call(name, args, timeout=60):
    ...
    res, _ = rpc(call, sid, timeout=timeout)
...
for attempt in range(3):
    print("attempt", attempt, "->", refl("ForceGather"))
    time.sleep(1.2)
    dump = refl("Dump")
    print(dump)
    if "chibi@" in dump:
        ...
        if not shot.get("isError"):
            for item in shot.get("content", []):
                if item.get("type")=="image":
                    ...
                    print("SAVED TestEvidence/step3_testC_spine_walk.png", len(png), "bytes")
                    sys.exit(0)
        else:
            print("screenshot failed"); sys.exit(1)
    time.sleep(2)
print("NO_CHIBI_CAUGHT")
