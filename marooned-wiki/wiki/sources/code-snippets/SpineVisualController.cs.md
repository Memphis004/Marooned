---
title: SpineVisualController
type: snippet
sources: ["[[sources/spinevisualcontroller-cs]]"]
related:
  - "[[IChibiVisual.cs]]"
  - "[[ChibiSpawnerView.cs]]"
  - "[[chibi-visual-system]]"
  - SkeletonAnimation (Spine-Unity 4.3)
folder: Core/Visual
lines: 112
created: 2026-09-06
tags:
...
**Path:** `Marooned/Assets/Scripts/Core/Visual/SpineVisualController.cs`

Spine backend (experimental) ของระบบ chibi — controller ตัวเดียวใช้ร่วมกันทั้ง
Elena และ Derek (ผ่าน prefab variant `ElenaChibi`/`DerekChibi` ที่ต่างกันแค่
SkeletonDataAsset) ชื่อ animation map จาก SkeletonData จริง 29 ชื่อเหมือนกันทั้งสองตัว
ข้อควรระวัง Spine 4.3: `GetCurrent(track)` → **`GetTrack(track)`**, flip ด้วย
`Skeleton.ScaleX` ดูสถาปัตยกรรมรวมที่ [[chibi-visual-system]]
