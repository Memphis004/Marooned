---
title: RoundInitializer
type: snippet
sources: ["[[sources/roundinitializer-cs]]"]
related:
  - "[[NpcDirectorSystem.cs]]"
  - "[[GameLifetimeScope.cs]]"
  - "[[GameStateProvider.cs|LubanDataService]]"
  - "[[LocationDef]]"
  - "[[GameTickDriver.cs]]"
  - VContainer
folder: Core
...
# RoundInitializer.cs
**Path:** `Marooned/Assets/Scripts/Core/RoundInitializer.cs`
...
## Purpose
เรียก `NpcDirectorSystem.SetupRound()` อัตโนมัติเมื่อ Scene โหลดเสร็จ — แก้ปัญหา
"ไม่มี NPC เกิดในเกม" ที่ทำให้ MCP Tools (`get_visible_npcs`, `call_meeting`,
`accuse_npc`) คืนค่าว่างเสมอ

## Public API
| Member | คำอธิบาย |
| --- | --- |
| `[SerializeField] int npcCount = 5` | จำนวน NPC ต่อรอบ (Inspector ปรับได้) |
| `[SerializeField] int killerCount = 1` | จำนวน killer ต่อรอบ (อัตราส่วน 5→1 ตาม comment ใน NpcDirectorSystem) |

(ทั้งหมดทำงานผ่าน lifecycle `Start()` — ไม่มี method ให้เรียกจากภายนอก)

## Dependencies
- **GameLifetimeScope** (VContainer composition root) — หาด้วย
  `GetComponentInParent<>()` แล้ว Resolve ระบบจาก `Container` ทั้งหมดใน code
  (ไม่ลาก reference ใน Inspector)
- **NpcDirectorSystem** — รับ `SetupRound()` + `Npcs` dictionary
- **GameStateProvider** — อ่าน `Player.CurrentLocationId` เป็นจุดวาง NPC
- **LubanDataService** — อ่าน `LocationDefs` หา location ที่เชื่อมกัน

## Key Logic
- **Resolve ใน `Start()` ไม่ใช่ `Awake()`**: container ถูก build ใน
  `LifetimeScope.Awake` — สคริปต์นี้ติดเป็นลูกของ GameLifetimeScope จึงต้องรอ
  parent Awake เสร็จ (Start รันหลัง Awake ทุกตัวเสมอ จึงปลอดภัยจาก race)
- **Guard กันซ้ำ**: ถ้า `Npcs.Count > 0` ข้าม (กันเริ่มรอบซ้อน)
- **สร้าง roster**: `npc_01` … `npc_05` (hardcode mock — รอตาราง Luban NpcDef)
- **`AssignStartingLocations()`** — จุดสำคัญที่สุด: `SetupRound()` ของ
  NpcDirectorSystem **ไม่ได้**กำหนด `CurrentLocationId` ให้ NPC (คงเป็น null)
  แต่ `DeductionSystem.GetObservableNpcsAt()` กรองด้วย `CurrentLocationId ==
  locationId` ดังนั้นถ้าไม่วาง NPC ลง location `get_visible_npcs` จะยังว่าง
  อยู่เสมอแม้ SetupRound ถูกเรียกแล้ว
  - 2 ตัวแรกวางที่ location ผู้เล่น (เจอคนทันที)
  - ที่เหลือ round-robin ตาม `ConnectedLocationIds` ของ location ผู้เล่น

## TODO / Known Issues
- roster `npc_XX` hardcode — ย้ายไปอ่านจาก Luban table (NpcDef) เมื่อมี
- การวางตำแหน่งเริ่มต้นควรย้ายเข้า `SetupRound()` เอง (พร้อม data-driven
  spawn table) เมื่อระบบรอบเกมถูกปรับ — ตอนนี้ทำใน Initializer เพื่อไม่แก้ระบบเดิม
- `killerCount` ยังไม่มี formula auto-calc (ดู GDD §2.3 ผ่าน
  [[NpcDirectorSystem.cs]] Known Issues)
